...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: `${area.title} | Işık Hukuk Bürosu`, description: area.description },
  };
}
...
export default async function PracticeAreaDetailPage({ params }: Props) {
  ...
  const articlesData = await getArticles({ categorySlug: slug, pageSize: 6 }).catch(() => null);
    ...
    <main>
        ...
        <div className="mx-auto max-w-5xl px-6">
          <nav aria-label="Breadcrumb" className="mb-8">
            <ol className="flex flex-wrap items-center gap-1.5 text-xs text-gray-400">
              ...
              <li><Link href="/calisma-alanlari" className="hover:text-white transition-colors">Çalışma Alanları</Link></li>
              <li><ChevronRight size={12} /></li>
      ...
      <section className="py-20">
        <div className="mx-auto max-w-5xl px-6">
          {articles.length > 0 ? (
            <>
              <h2 className="mb-10 font-serif text-2xl font-light text-navy">
                Bu Alandaki Yazılar
              </h2>
            ...
            <div className="rounded-none border border-gray-100 bg-surface px-8 py-12 text-center">
              <p className="text-gray-500">
                Bu alana ait henüz yayınlanmış makale bulunmamaktadır.
              </p>
              <Link
                href="/blog"
