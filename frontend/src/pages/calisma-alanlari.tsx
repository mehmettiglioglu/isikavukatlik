    ...
    <main>
        ...
        <div className="relative z-10 mx-auto max-w-5xl px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <p className="mt-4 max-w-xl text-base leading-relaxed text-gray-300">
              Büromuz, hukukun farklı dallarında uzmanlaşmış kadrosuyla müvekkillerimize kapsamlı danışmanlık ve temsil hizmeti sunmaktadır.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20">
        <div className="mx-auto max-w-5xl px-6">
          <motion.ul className="grid gap-4 sm:grid-cols-2" role="list" initial="hidden" animate="visible" variants={staggerContainer}>
            {PRACTICE_AREAS.map((area, i) => (
              <motion.li key={area.slug} variants={fadeUp} custom={i}>
                <Link to={`/calisma-alanlari/${area.slug}`} className="group flex items-start gap-5 border border-gray-100 bg-white p-6 transition-all duration-300 hover:border-gold/40 hover:shadow-md">
                  ...
                  <div className="flex-1 min-w-0">
                    <h2 className="font-medium text-[#2d2d2d] leading-snug transition-colors group-hover:text-navy">{area.title}</h2>
                    <p className="mt-1.5 text-sm leading-relaxed text-gray-500">{area.description}</p>
                  </div>
                  <ArrowRight size={16} className="mt-1 shrink-0 text-gold opacity-0 transition-all group-hover:opacity-100 group-hover:translate-x-1" />
                </Link>
              </motion.li>
            ))}
          </motion.ul>
