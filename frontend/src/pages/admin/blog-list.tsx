    ...
    <div>
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-400">{total} makale &mdash; {published} yayında, {draft} taslak</p>
        </div>
        <Link to="/admin/blog/yeni" className="inline-flex items-center gap-2 bg-navy px-5 py-2.5 text-sm font-medium text-white transition-opacity hover:opacity-90">
      ...
      <div className="mb-4 relative max-w-sm">
        ...
        <input type="search" placeholder="Makale veya kategori ara..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full border border-gray-200 bg-white py-2.5 pl-10 pr-4 text-sm outline-none transition-colors focus:border-navy" />
      </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((article) => (
                <tr key={article.id} onClick={() => navigate(`/admin/blog/${article.id}/duzenle`)} className="group cursor-pointer transition-colors hover:bg-gray-50/60">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      {article.coverImageUrl && (
                        <img src={article.coverImageUrl} alt="" className="h-9 w-14 shrink-0 object-cover" />
                      )}
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`inline-flex items-center gap-1.5 text-xs font-medium ${article.isPublished ? "text-green-700" : "text-amber-600"}`}>
                      {article.isPublished ? <><Eye size={12} /> Yayında</> : <><EyeOff size={12} /> Taslak</>}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-3.5" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <Link to={`/admin/blog/${article.id}/duzenle`} className="p-1.5 text-gray-400 transition-colors hover:text-navy" title="Düzenle">
                        <Pencil size={15} />
                      </Link>
                      <button onClick={() => handleDelete(article.id, article.title)} disabled={deleting === article.id} className="p-1.5 text-gray-400 transition-colors hover:text-red-500 disabled:opacity-40" title="Sil">
                        {deleting === article.id
